...
module Ecm::Pictures
  describe Picture do
    ...
    context 'setting default handler' do
      ...
      it { expect(subject.markup_language).to eq(Ecm::Pictures::Configuration.default_markup_language.to_s) }
    ...
    context '#display_code' do
      ...
      context 'for textile' do
        it 'should generate the correct code' do
          subject.display_code(:textile).should eq("!/system/ecm/pictures/pictures/images/#{subject.id.to_s.rjust(9, '0').scan(/.{1,3}/).join('/')}/original/image.jpg!")
        end # it
