namespace :ecm do
  namespace :pictures do
    namespace :db do
      ...
      task :clear_galleries!, [] => [:environment] do |_t, _args|
        Ecm::Pictures::PictureGallery.delete_all
      end

      desc 'Clears all picture data!'
      task :clear_pictures!, [] => [:environment] do |_t, _args|
      ...
      task :populate_galleries, [] => [:environment] do |_t, _args|
        require 'ffaker'
        5.times do |i|
      ...
      task :populate_pictures, [] => [:environment] do |_t, _args|
        require 'ffaker'
        require 'forgery'
